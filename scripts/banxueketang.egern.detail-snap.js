  ...
  try {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        // 【快照】详情类接口：输出响应中所有权益相关字段
        try {
          const u = ctx.request.url;
          if (/getMaterialBusinessDetail|VideoDetail|video\/detail|albumDetail|play|player|businessDetail/i.test(u)) {
            ctx.notify({ title: '详情接口响应', body: u.slice(0, 90) + '\n' + snapshotRights(data) });
          }
        } catch (e) {}
        unlock(data);
...
/* 【快照】深度提取响应中所有权益/投屏相关字段 */
function snapshotRights(data) {
  const hits = [];
  walk(data, '', 0);
  return hits.slice(0, 8).join('\n').slice(0, 320) || '(无权益相关字段)';

  function walk(obj, path, depth) {
    if (!obj || typeof obj !== 'object' || depth > 5 || hits.length >= 8) return;
    if (Object.prototype.toString.call(obj) === '[object Array]') {
      for (let i = 0; i < obj.length && hits.length < 8; i++) walk(obj[i], path + '[' + i + ']', depth + 1);
      return;
    }
    for (const k in obj) {
      if (hits.length >= 8) return;
      const v = obj[k];
      if (/(isRight|cast|vip|screen|lock|allow|purchase|buy|member|sku|func)/i.test(k)) {
        hits.push(path + '.' + k + '=' + JSON.stringify(v).slice(0, 80));
      }
