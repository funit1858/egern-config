  ...
  try {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        ...
        // 【资料调试】document-library 接口：输出 libraryUrl/isHave 等字段
        try {
          if (ctx.request.url.indexOf('document-library') >= 0) {
            ctx.notify({ title: '资料接口', body: ctx.request.url.slice(0, 90) + '\n' + materialSnap(data) });
          }
        } catch (e) {}
...
/* 【资料调试】提取资料相关字段 */
function materialSnap(data) {
  const hits = [];
  walk(data, '', 0);
  return hits.slice(0, 10).join('\n').slice(0, 340) || '(无匹配字段)';
  function walk(obj, path, depth) {
    if (!obj || typeof obj !== 'object' || depth > 5 || hits.length >= 10) return;
    if (Object.prototype.toString.call(obj) === '[object Array]') {
      for (let i = 0; i < obj.length && hits.length < 10; i++) walk(obj[i], path + '[' + i + ']', depth + 1);
      return;
    }
    for (const k in obj) {
      if (hits.length >= 10) return;
      const v = obj[k];
      if (/(libraryUrl|libraryName|isHave|isLock|isVip|price|amount|exchange|material)/i.test(k)) {
        hits.push(path + '.' + k + '=' + JSON.stringify(v).slice(0, 90));
      }
