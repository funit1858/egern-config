...
function summarizeSku(data) {
  try {
    ...
    const list = Array.isArray(raw) ? raw : (raw && Array.isArray(raw.list) ? raw.list : [raw]);
    const parts = [];
    for (let i = 0; i < Math.min(list.length, 3); i++) {
      const item = list[i];
      if (!item || typeof item !== 'object') { parts.push('item=' + JSON.stringify(item).slice(0, 80)); continue; }
      parts.push(
        'dictCode=' + (item.dictCode || '?') +
        ' keys=[' + Object.keys(item).join(',') + ']' +
        ' sku={' + JSON.stringify(item.skuVipRightsApiDTO || {}).slice(0, 110) + '}' +
        ' usr={' + JSON.stringify(item.appUserVipRightsApiDTO || {}).slice(0, 110) + '}'
      );
    }
    return parts.join('\n').slice(0, 380);
  } catch (e) {
