 ...
 *   - 但前端还有明文接口 /app-api/userAuth/checkUserAndLoginRead
 *     只要登录（UID+USER_LOGIN_CODE）就返回 code:0 + articleProperties.content 完整正文
 *     （服务端只校验登录状态，不校验订阅）
 *   - 方案：在 articlev5 页面注入 JS，用请求头 cookie 里的 UID/code 直接 fetch 该接口，
 *     把正文渲染进 #cx-cons 容器并标记 body.expand，绕过付费墙
...
export default async function (ctx) {
  try {
    ...
    if (/mappsv5\.caixin\.com\/articlev5\//.test(url)) {
      ...
      // 从请求头 cookie 提取 UID / USER_LOGIN_CODE
      const headers = ctx.request.headers || {};
      const cookie = (headers['cookie'] || headers['Cookie'] || '') + ';' +
                     (headers['Cookie2'] || '') + ';' +
                     (headers['cookie2'] || '');
      const uidMatch = cookie.match(/SA_USER_UID=(\d+)/) || cookie.match(/UID=(\d+)/);
      const codeMatch = cookie.match(/USER_LOGIN_CODE=([^;]+)/);
      const uid = uidMatch ? uidMatch[1] : '';
      const code = codeMatch ? codeMatch[1] : '';

      // 注入的解锁 JS（uid/code 直接内联，不依赖 document.cookie）
      const inject = `
<script>
(function() {
