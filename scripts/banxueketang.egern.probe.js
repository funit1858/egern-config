  ...
  try {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        // 【探测】响应里带权益字段 → 弹通知（找出投屏状态数据源）
        try {
          const hit = probeRights(data, ctx.request.url);
          if (hit) ctx.notify({ title: '权益数据接口', body: hit });
        } catch (e) {}
        unlock(data);
...
/* 【探测】深度查找权益相关字段，返回 URL + 命中摘要 */
function probeRights(data, url) {
  const hits = [];
  walk(data, '', 0);
  ...
  function walk(obj, path, depth) {
    ...
    for (const k in obj) {
      ...
      const v = obj[k];
      // 命中：权益相关字段（含值）
      if (/^(isRights|isRightsAvailable|funcSkuData|skuVipRightsApiDTO|appUserVipRightsApiDTO|dictCode|videoTVScreen|castRights|canCast|isVip)$/.test(k)) {
        hits.push(path + '.' + k + '=' + JSON.stringify(v).slice(0, 60));
      }
